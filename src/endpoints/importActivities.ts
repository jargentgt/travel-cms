...
const importActivities: Endpoint = {
  ...
  handler: async (req) => {
    try {
      ...
      // Update trip with organized days (merge with existing days)
      const existingDays = trip.days || []
      const existingDaysByDate: { [key: string]: any } = {}
      ...
      // Merge new activities with existing days
      Object.entries(dayGroups).forEach(([dateKey, activityIds]) => {
        if (existingDaysByDate[dateKey]) {
          // Add to existing day
          existingDaysByDate[dateKey].activities = [
            ...(existingDaysByDate[dateKey].activities || []),
            ...activityIds
          ]
        } else {
          // Create new day
          existingDaysByDate[dateKey] = {
            date: dateKey,
      ...
      const updatedDays = Object.values(existingDaysByDate)
